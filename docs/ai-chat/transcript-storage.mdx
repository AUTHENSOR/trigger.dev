...
Set `storage` on the agent to persist the conversation yourself, one row per message:
...
`save` keeps receiving every change, and crash recovery keeps running. This is the replacement for the deprecated [`hydrateMessages`](/ai-chat/lifecycle-hooks#hydratemessages) hook, and it cannot be combined with `storage`.
